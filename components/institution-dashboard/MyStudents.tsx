  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className={`${heading} font-bold text-xl`}>My Students</h2>
          <p className="text-sm mt-1" style={{ color: "var(--muted)" }}>{students.length} students enrolled</p>
        </div>
        <button onClick={() => onNavigate("add-student")} className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm`} style={{ background: "var(--primary)", color: "white" }}>+ Add Student</button>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}>
          ...
          <p className={`${heading} font-bold text-base mb-1`}>{students.length === 0 ? "No students yet" : "No results"}</p>
            ...
            {filtered.map((s) => (
              <div key={s.userId} className="flex items-center gap-4 px-5 py-4 hover:bg-gray-50 transition-colors">
                ...
                <div className="flex-1 min-w-0">
                  <div className={`${heading} font-bold text-sm`}>{s.user.name}</div>
                ...
                <div className="flex gap-2 shrink-0">
                  <Link href={`/profile/${s.profileNumber}`} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem] no-underline`} style={{ background: "var(--primary)", color: "white" }}>View</Link>
