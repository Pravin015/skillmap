  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h2 className={`${heading} font-bold text-xl`}>Competitions & Challenges</h2>
          <p className="text-xs mt-0.5" style={{ color: "var(--muted)" }}>Create hackathons, coding challenges, quizzes, and case studies</p>
        </div>
        <button onClick={() => setShowForm(!showForm)} className={`${heading} text-xs font-bold px-4 py-2 rounded-xl`} style={{ background: "var(--primary)", color: "white" }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} className="rounded-2xl border bg-white p-6 space-y-4" style={{ borderColor: "var(--border)" }}>
          ...
          <button type="submit" disabled={saving} className={`${heading} rounded-xl px-6 py-2.5 text-sm font-bold disabled:opacity-50`} style={{ background: "var(--primary)", color: "white" }}>{saving ? "Creating..." : "Create Competition"}</button>
        </form>
      )}

      {/* My Competitions */}
      <div>
        <h3 className={`${heading} text-sm font-bold mb-3`}>Your Competitions ({myComps.length})</h3>
        {myComps.length === 0 ? (
            ...
            {myComps.map((c) => (
              <div key={c.id} className="rounded-xl border bg-white p-4 flex items-center gap-3" style={{ borderColor: "var(--border)" }}>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2"><span className={`${heading} text-sm font-bold`} style={{ color: "var(--ink)" }}>{c.title}</span><span className="rounded-full px-2 py-0.5 text-[9px] font-medium" style={{ background: c.status === "OPEN" || c.status === "LIVE" ? "#10b98115" : "#6b728015", color: c.status === "OPEN" || c.status === "LIVE" ? "#10b981" : "#6b7280" }}>{c.status}</span></div>
