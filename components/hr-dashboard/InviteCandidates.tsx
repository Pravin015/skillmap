...
export default function InviteCandidates() {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className={`${heading} font-bold text-xl`}>Sent Invitations</h2><p className="text-sm mt-1" style={{ color: "var(--muted)" }}>{invites.length} invites sent · Use <strong>AI JD Match</strong> tab to find and invite candidates</p></div>

      {invites.length === 0 ? (
        <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}>
          <div className="text-4xl mb-3">✉️</div>
          <p className={`${heading} font-bold text-base mb-1`}>No invites sent yet</p>
        ...
        <div className="space-y-3">
          {invites.map((inv) => (
            <div key={inv.id} className="rounded-2xl border bg-white p-5 flex items-center gap-4" style={{ borderColor: "var(--border)" }}>
              <div className={`w-10 h-10 rounded-full flex items-center justify-center ${heading} font-bold text-xs text-white shrink-0`} style={{ background: "var(--ink)" }}>{inv.candidateName?.charAt(0) || "?"}</div>
              <div className="flex-1 min-w-0">
                <div className={`${heading} font-bold text-sm`}>{inv.candidateName}</div>
