  ...
  return (
    <div className="relative">
      <button onClick={handleOpen} className="relative p-2 rounded-lg transition-colors hover:bg-[rgba(10,10,15,0.04)]">
        ...
        {count > 0 && (
          <span className={`absolute -top-0.5 -right-0.5 flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[0.55rem] font-bold ${syne}`} style={{ background: "var(--primary)", color: "white" }}>
            {count > 9 ? "9+" : count}
          </span>
        )}
      </button>
