...
export default function CompanyOverview({ orgName, hrCount, onNavigate }: Props) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className={`${syne} font-extrabold text-xl`}>{orgName} Dashboard</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-3 gap-4">
        {statCards.map((s) => (
          <div key={s.key} className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
            ...
            <div className={`${syne} text-2xl font-extrabold`}>{stats[s.key]}</div>
      ...
      {/* Hiring funnel */}
      <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
        <h3 className={`${syne} font-bold text-base mb-2`}>Hiring Funnel</h3>
