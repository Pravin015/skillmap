...
export default function CompanySettings({ orgName }: { orgName: string }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className={`${syne} font-bold text-xl`}>Company Settings</h2>
        <p className="text-sm mt-1" style={{ color: "var(--muted)" }}>Manage your organisation profile and preferences</p>
      </div>

      <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
        <h3 className={`${syne} font-bold text-base mb-4`}>Organisation Profile</h3>
        <div className="flex items-center gap-4 mb-6">
          <div className={`w-16 h-16 rounded-2xl flex items-center justify-center ${syne} font-extrabold text-xl text-white`} style={{ background: "var(--ink)" }}>
            {orgName.charAt(0)}
          </div>
          <div>
            <div className={`${syne} font-bold text-lg`}>{orgName}</div>
      ...
      <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
        <h3 className={`${syne} font-bold text-base mb-2`}>Notification Preferences</h3>
        ...
        <div className="space-y-3">
          {[
            ...
            <label key={n.label} className="flex items-center justify-between p-3 rounded-xl border cursor-pointer hover:bg-gray-50" style={{ borderColor: "var(--border)" }}>
              <div>
                <div className={`${syne} font-bold text-sm`}>{n.label}</div>
