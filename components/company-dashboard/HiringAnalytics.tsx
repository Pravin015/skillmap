...
export default function HiringAnalytics({ hrCount }: { hrCount: number }) {
  return (
    <div className="space-y-6">
      <div>
        <h2 className={`${syne} font-bold text-xl`}>Hiring Analytics</h2>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
        {[
          ...
          <div key={m.label} className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
            ...
            <div className={`${syne} text-2xl font-extrabold`}>{m.value}<span className="text-xs font-normal ml-1" style={{ color: "var(--muted)" }}>{m.sub}</span></div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
          <h3 className={`${syne} font-bold text-base mb-2`}>Hiring Trend</h3>
        ...
        <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
          <h3 className={`${syne} font-bold text-base mb-2`}>Source of Hires</h3>
      ...
      {/* Domain breakdown */}
      <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
        <h3 className={`${syne} font-bold text-base mb-2`}>Hiring by Domain</h3>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
          {["Software", "Cybersecurity", "Cloud", "Data", "Consulting"].map((d) => (
            <div key={d} className="rounded-xl border p-4 text-center" style={{ borderColor: "var(--border)" }}>
              <div className={`${syne} text-lg font-extrabold`} style={{ color: "var(--border)" }}>0</div>
      ...
      {/* Candidate quality */}
      <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
        <h3 className={`${syne} font-bold text-base mb-2`}>Candidate Quality Score</h3>
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="rounded-xl border-2 border-dashed p-6 text-center" style={{ borderColor: "var(--border)" }}>
            <div className={`${syne} text-3xl font-extrabold`} style={{ color: "var(--border)" }}>—</div>
