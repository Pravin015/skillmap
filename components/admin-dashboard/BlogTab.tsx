  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div><h2 className={`${heading} font-bold text-xl`}>Blog Management</h2><p className="text-sm mt-1" style={{ color: "var(--muted)" }}>{posts.length} posts · {pending} pending review</p></div>
        <Link href="/blog/write" className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm no-underline`} style={{ background: "var(--primary)", color: "white" }}>+ Write Post</Link>
      </div>
      ...
      {filtered.length === 0 ? (
        <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}><div className="text-4xl mb-3">📝</div><p className={`${heading} font-bold text-base`}>No posts</p><Link href="/blog/write" className={`inline-block mt-3 px-4 py-2 rounded-lg ${heading} font-bold text-xs no-underline`} style={{ background: "var(--primary)", color: "white" }}>Write first post</Link></div>
      ) : (
          ...
          {filtered.map((p) => (
            <div key={p.slug} className="rounded-2xl border bg-white p-5 flex items-center gap-4 flex-wrap" style={{ borderColor: "var(--border)" }}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2"><span className={`${heading} font-bold`}>{p.title}</span><span className={`text-[0.6rem] font-bold px-2 py-0.5 rounded-full ${statusBadge[p.status] || ""}`}>{p.status.replace("_", " ")}</span></div>
              ...
              <div className="flex gap-2 shrink-0">
                {p.status === "PENDING_REVIEW" && (
                  <><button onClick={() => handleAction(p.slug, "approve")} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem]`} style={{ background: "var(--primary)", color: "white" }}>Publish</button>
