    ...
    <div className="space-y-6">
      ...
      {showCreate && (
        <form onSubmit={handleCreateLab} className="rounded-2xl border bg-white p-6 space-y-4" style={{ borderColor: "var(--border)" }}>
          <h3 className={`${syne} font-bold text-base`}>New Lab Template</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div><label className={`block text-sm font-medium mb-1 ${syne}`}>Title *</label><input name="title" required placeholder="e.g. Cybersecurity Basics" className={inputClass} style={{ borderColor: "var(--border)" }} /></div>
            <div><label className={`block text-sm font-medium mb-1 ${syne}`}>Domain *</label><select name="domain" required className={inputClass} style={{ borderColor: "var(--border)" }}><option value="">Select</option><option>Software Development</option><option>Cybersecurity</option><option>Cloud & DevOps</option><option>Data & Analytics</option><option>Consulting</option></select></div>
            <div><label className={`block text-sm font-medium mb-1 ${syne}`}>Difficulty</label><select name="difficulty" className={inputClass} style={{ borderColor: "var(--border)" }}><option>EASY</option><option>MEDIUM</option><option>HARD</option></select></div>
            <div><label className={`block text-sm font-medium mb-1 ${syne}`}>Time Limit (min)</label><input name="timeLimit" type="number" defaultValue={30} min={5} className={inputClass} style={{ borderColor: "var(--border)" }} /></div>
      ...
      {editLabId && (
        <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
          <div className="flex items-center justify-between mb-4"><h3 className={`${syne} font-bold text-base`}>Edit Problems</h3><button onClick={() => setEditLabId(null)} className="text-xs" style={{ color: "var(--muted)" }}>Close</button></div>
          <div className="space-y-4">
            {problems.map((p, i) => (
              <div key={i} className="rounded-xl border p-4" style={{ borderColor: "var(--border)" }}>
                <div className={`${syne} font-bold text-xs mb-2`}>Q{i + 1}</div>
      ...
      {labs.length === 0 && !showCreate ? (
        <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}><div className="text-4xl mb-3">🧪</div><p className={`${syne} font-bold text-base`}>No labs created</p></div>
      ) : (
          ...
          {labs.map((lab) => (
            <div key={lab.id} className="rounded-2xl border bg-white p-5 flex items-center gap-4 flex-wrap" style={{ borderColor: "var(--border)" }}>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2"><span className={`${syne} font-bold`}>{lab.title}</span><span className={`text-[0.6rem] font-bold px-2 py-0.5 rounded-full ${lab.status === "PUBLISHED" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>{lab.status}</span><span className="text-[0.6rem] px-2 py-0.5 rounded-full border" style={{ borderColor: "var(--border)", color: "var(--muted)" }}>{lab.difficulty}</span></div>
