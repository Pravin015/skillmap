...
export default function AddUserTab({ onRefresh }: { onRefresh: () => void }) {
  ...
  return (
    <div className="space-y-6">
      <div><h2 className={`${heading} font-bold text-xl`}>Add User</h2><p className="text-sm mt-1" style={{ color: "var(--muted)" }}>Create any type of user account</p></div>

      <div className="flex gap-2 flex-wrap">
        {["STUDENT", "HR", "ORG", "INSTITUTION", "MENTOR", "ADMIN"].map((r) => (
          <button key={r} onClick={() => setRole(r)} className={`px-4 py-2 rounded-xl text-xs ${heading} font-bold`} style={{ background: role === r ? "var(--ink)" : "white", color: role === r ? "var(--primary)" : "var(--muted)", border: role === r ? "none" : "1px solid var(--border)" }}>{r}</button>
        ))}
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-2xl border bg-white p-6 space-y-5" style={{ borderColor: "var(--border)" }}>
        ...
        <p className="text-xs" style={{ color: "var(--muted)" }}>Creating as: <strong className={heading}>{role}</strong>. A temporary password will be auto-generated.</p>
        <button type="submit" disabled={saving} className={`px-6 py-3 rounded-xl ${heading} font-bold text-sm disabled:opacity-50`} style={{ background: "var(--primary)", color: "white" }}>{saving ? "Creating..." : `Create ${role} Account`}</button>
      </form>
    </div>
  );
}
