    ...
    <div>
      ...
      <section className="relative px-4 flex flex-col items-center justify-center text-center" style={{ background: "#0F0E14", minHeight: "85vh", paddingTop: "7rem", paddingBottom: "5rem" }}>
        ...
        <div className="relative max-w-[750px] mx-auto w-full">
          ...
          <p className="animate-fade-up-2 mx-auto" style={{ color: "#6B6776", fontSize: "clamp(1rem, 2vw, 1.15rem)", lineHeight: 1.75, maxWidth: 580, marginBottom: "2.5rem" }}>AI-powered candidate matching, proctored assessments, and hiring challenges — all in one platform. Find candidates who can actually do the job.</p>
          ...
          <div className="animate-fade-up-3 mb-10">
            <p className="mb-3" style={{ color: "#9A95A6", fontSize: "0.8rem" }}>Trusted by hiring teams at</p>
      ...
      <section className="px-4" style={{ background: "#fff", paddingTop: "6rem", paddingBottom: "6rem" }}>
        <div className="mx-auto" style={{ maxWidth: 1200 }}>
          <div className="text-center mb-12">
            ...
            <h2 className={heading+" animate-on-scroll"} style={{color:"#0F0E14"}}>End-to-End Hiring.</h2>
            <p className="animate-on-scroll mt-2 mx-auto" style={{color:"#9A95A6",fontSize:"1rem",maxWidth:500}}>From job post to offer letter — one platform.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            {/* AI JD Match */}
            <div className="card-dark animate-on-scroll" style={{borderTop:"2px solid #7C3AED",padding:"2.5rem"}}>
              <div className="flex items-center gap-3 mb-4">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#7C3AED" strokeWidth="1.5"><circle cx="11" cy="11" r="8"/><path d="M21 21l-4.35-4.35"/></svg>
                <span style={{background:"rgba(124,58,237,0.12)",border:"1px solid rgba(124,58,237,0.25)",borderRadius:999,padding:"0.2rem 0.7rem",fontSize:"0.7rem",fontWeight:600,color:"#7C3AED"}}>Powered by Claude AI</span>
              </div>
              ...
              <p style={{color:"#6B6776",fontSize:"0.9rem",lineHeight:1.65,marginBottom:"1.5rem"}}>Paste any job description. Our AI scans every candidate profile and returns the top matches with percentage scores. No more manually reviewing 500 resumes.</p>
              <div style={{borderRadius:"0.75rem",padding:"1rem",background:"rgba(255,255,255,0.03)"}}>
                <div style={{fontSize:"0.7rem",color:"#6B6776",marginBottom:"0.75rem",fontWeight:600}}>AI Match Results</div>
                {[{n:"Arjun M.",p:94},{n:"Priya S.",p:87},{n:"Rahul K.",p:81}].map((c)=>(<div key={c.n} className="flex items-center gap-3 mb-2"><div style={{width:"1.75rem",height:"1.75rem",borderRadius:"50%",background:"rgba(255,255,255,0.08)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:"0.6rem",color:"rgba(255,255,255,0.5)",fontWeight:600}}>{c.n.charAt(0)}</div><span style={{color:"rgba(255,255,255,0.7)",fontSize:"0.8rem",width:65}}>{c.n}</span><div className="flex-1" style={{height:4,borderRadius:2,background:"rgba(255,255,255,0.06)"}}><div style={{height:4,borderRadius:2,width:c.p+"%",background:"#7C3AED"}}/></div><span style={{color:"#7C3AED",fontSize:"0.75rem",fontWeight:700}}>{c.p}%</span></div>))}
              </div>
            </div>
            {/* Pipeline */}
            <div className="card-dark animate-on-scroll" style={{borderTop:"2px solid #F59E0B",padding:"2.5rem"}}>
              ...
              <p style={{color:"#6B6776",fontSize:"0.9rem",lineHeight:1.65,marginBottom:"1.5rem"}}>Drag candidates through your hiring funnel. Applied, Screening, Interview, Assessment, Offer, Hired. No spreadsheets. No lost candidates.</p>
              <div className="flex gap-2" style={{borderRadius:"0.75rem",padding:"1rem",background:"rgba(255,255,255,0.03)"}}>
                {[{t:"Applied",c:2,h:false},{t:"Interview",c:1,h:false},{t:"Offered",c:1,h:true}].map((col)=>(<div key={col.t} className="flex-1"><div style={{fontSize:"0.65rem",color:"#6B6776",marginBottom:"0.4rem",fontWeight:600}}>{col.t}</div>{Array.from({length:col.c}).map((_,i)=>(<div key={i} style={{background:col.h?"rgba(124,58,237,0.15)":"rgba(255,255,255,0.05)",border:"1px solid "+(col.h?"rgba(124,58,237,0.3)":"rgba(255,255,255,0.08)"),borderRadius:"0.4rem",padding:"0.4rem 0.5rem",marginBottom:"0.3rem",fontSize:"0.65rem",color:col.h?"#7C3AED":"rgba(255,255,255,0.5)"}}>Candidate {i+1}</div>))}</div>))}
              </div>
            </div>
          </div>
          {/* Tier 2 */}
      ...
      {/* HOW IT WORKS */}
      <section id="how-co" style={{background:"#0F0E14",paddingTop:"6rem",paddingBottom:"6rem"}}>
        <div className="mx-auto px-4" style={{maxWidth:1000}}>
          <div className="text-center mb-14">
            ...
            <p className="animate-on-scroll" style={{color:"#9A95A6",marginTop:"0.5rem"}}>Set up in under 10 minutes.</p>
          </div>
          <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 lg:gap-6">
              {[{n:"01",t:"Register Your Company",d:"Create your org account. Set up your company profile with branding and culture info. Free."},{n:"02",t:"Add Your HR Team",d:"Invite HR members. Each gets their own dashboard with job posting and candidate management."},{n:"03",t:"Post Jobs & Challenges",d:"Create job openings with optional proctored lab assessments. Launch hiring challenges."},{n:"04",t:"Hire Top Candidates",d:"Review AI-matched candidates, use the Kanban pipeline, close hires faster."}].map((s)=>(<div key={s.n} className="text-center animate-on-scroll"><div className="mx-auto flex items-center justify-center" style={{width:"3rem",height:"3rem",borderRadius:"50%",background:"rgba(255,255,255,0.05)",border:"1.5px solid rgba(255,255,255,0.2)"}}><span className={heading} style={{fontSize:"0.9rem",fontWeight:700,color:"#7C3AED"}}>{s.n}</span></div><h3 className={heading} style={{color:"#fff",fontWeight:700,fontSize:"1.1rem",marginTop:"1.25rem",marginBottom:"0.5rem"}}>{s.t}</h3><p className="mx-auto" style={{color:"#9A95A6",fontSize:"0.9rem",lineHeight:1.6,maxWidth:220}}>{s.d}</p></div>))}
            </div>
          </div>
          <div className="text-center mt-12"><p style={{color:"#6B6776",fontSize:"0.9rem",marginBottom:"1rem"}}>Ready to hire smarter?</p><Link href="/auth/signup?role=ORG" className="btn-primary no-underline" style={{padding:"0.9rem 2rem",fontSize:"1rem"}}>Register Your Company — Free &#8594;</Link></div>
        </div>
      </section>

      {/* PRICING */}
      <section className="px-4" style={{background:"#F3EFE8",paddingTop:"6rem",paddingBottom:"6rem"}}>
        <div className="mx-auto" style={{maxWidth:1000}}>
          <div className="text-center mb-12">
            <div className="section-eyebrow justify-center animate-on-scroll">PRICING</div>
            <h2 className={heading+" animate-on-scroll"} style={{color:"#0F0E14"}}>Simple, Honest Pricing.</h2>
            <p className="animate-on-scroll mt-2" style={{color:"#9A95A6"}}>No hidden fees. No lock-in. Cancel anytime.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
            <div className="animate-on-scroll" style={{background:"#fff",border:"1px solid #E8E2D6",borderRadius:"1rem",padding:"2rem"}}><div className={heading} style={{fontWeight:700,color:"#0F0E14"}}>Starter</div><div className={heading} style={{fontWeight:700,fontSize:"2.5rem",color:"#7C3AED"}}>Free</div><p style={{color:"#9A95A6",fontSize:"0.85rem",marginBottom:"1.25rem"}}>For small teams getting started</p><div style={{borderTop:"1px solid #E8E2D6",margin:"1.25rem 0"}}/><ul className="space-y-2 mb-6">{["5 job posts/month","Basic candidate search","Application tracking","Email notifications"].map((f)=><li key={f} className="flex items-start gap-2 text-sm" style={{color:"#9A95A6"}}><span style={{color:"#7C3AED",fontWeight:700}}>&#10003;</span>{f}</li>)}</ul><Link href="/auth/signup?role=ORG" className="btn-outline block text-center no-underline w-full">Get Started</Link></div>
            <div className="animate-on-scroll relative card-dark" style={{padding:"2rem",transform:"translateY(-8px)"}}><div className="absolute -top-3 left-1/2 -translate-x-1/2" style={{background:"#7C3AED",color:"#fff",fontSize:"0.7rem",fontWeight:700,padding:"0.25rem 0.75rem",borderRadius:999}}>Most Popular</div><div className={heading} style={{fontWeight:700,color:"#fff"}}>Growth</div><div><span className={heading} style={{fontWeight:700,fontSize:"2.5rem",color:"#7C3AED"}}>Rs.4,999</span><span style={{color:"#9A95A6"}}>/month</span></div><p style={{color:"#9A95A6",fontSize:"0.85rem",marginBottom:"1.25rem"}}>For growing companies</p><div style={{borderTop:"1px solid rgba(255,255,255,0.08)",margin:"1.25rem 0"}}/><ul className="space-y-2 mb-6">{["Unlimited job posts","AI JD matching","Proctored assessments","Hiring challenges","Candidate pipeline","Priority support"].map((f)=><li key={f} className="flex items-start gap-2 text-sm" style={{color:"rgba(255,255,255,0.8)"}}><span style={{color:"#7C3AED",fontWeight:700}}>&#10003;</span>{f}</li>)}</ul><Link href="/auth/signup?role=ORG" className="btn-primary block text-center no-underline w-full animate-glow">Start Free Trial</Link></div>
            <div className="animate-on-scroll" style={{background:"#fff",border:"1px solid #E8E2D6",borderRadius:"1rem",padding:"2rem"}}><div className={heading} style={{fontWeight:700,color:"#0F0E14"}}>Enterprise</div><div className={heading} style={{fontWeight:700,fontSize:"2.5rem",color:"#7C3AED"}}>Custom</div><p style={{color:"#9A95A6",fontSize:"0.85rem",marginBottom:"1.25rem"}}>For large organisations</p><div style={{borderTop:"1px solid #E8E2D6",margin:"1.25rem 0"}}/><ul className="space-y-2 mb-6">{["Everything in Growth","Dedicated account manager","Custom integrations","SLA guarantee","White-label option","API access"].map((f)=><li key={f} className="flex items-start gap-2 text-sm" style={{color:"#9A95A6"}}><span style={{color:"#7C3AED",fontWeight:700}}>&#10003;</span>{f}</li>)}</ul><Link href="/forms/hire-from-us" className="btn-outline block text-center no-underline w-full">Contact Us</Link></div>
          </div>
        </div>
      </section>
      ...
      <section className="relative px-4 text-center" style={{background:"#0F0E14",paddingTop:"7rem",paddingBottom:"7rem"}}>
        ...
        <div className="relative mx-auto" style={{maxWidth:650}}>
          ...
          <h2 className={heading+" animate-on-scroll"} style={{fontWeight:700,fontSize:"clamp(2rem, 5vw, 3.5rem)",lineHeight:1.1,color:"#fff",marginBottom:"1rem"}}>Your Next Great Hire<br/>Is <span style={{color:"#7C3AED"}}>Already On AstraaHire.</span></h2>
          <p className="animate-on-scroll" style={{color:"#9A95A6",marginBottom:"2rem"}}>Register free. Post your first job in minutes.</p>
          <Link href="/auth/signup?role=ORG" className="btn-primary animate-glow no-underline" style={{padding:"0.9rem 2.2rem",fontSize:"1rem"}}>Register Your Company — Free &#8594;</Link>
          <div className="flex justify-center gap-2 mt-4 flex-wrap">{["&#128274; Secure","&#129302; AI-Matched","&#127470;&#127475; India&apos;s Platform"].map((b,i)=>(<span key={i} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:999,padding:"0.3rem 0.8rem",color:"#6B6776",fontSize:"0.75rem"}} dangerouslySetInnerHTML={{__html:b}}/>))}</div>
        </div>
      </section>
    </div>
  );
