    ...
    <div style={{ background: "var(--surface)" }}>
      ...
      <section className="px-4 py-16">
        <div className="max-w-5xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className={`${syne} font-extrabold text-2xl md:text-3xl`} style={{ color: "var(--ink)" }}>Everything You Need to Mentor</h2>
          </div>
      ...
      <section className="px-4 py-16" style={{ background: "var(--surface-alt)" }}>
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className={`${syne} font-extrabold text-2xl md:text-3xl`} style={{ color: "var(--ink)" }}>Your Knowledge Has Value</h2>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            {earningTiers.map((t) => (
              <div key={t.level} className="card-elevated text-center">
                <div className={`${syne} text-sm font-bold mb-1`} style={{ color: "var(--primary)" }}>{t.level}</div>
                <div className="text-[10px] mb-3" style={{ color: "var(--muted)" }}>{t.exp} experience</div>
                <div className={`${syne} text-xl font-extrabold mb-1`} style={{ color: "var(--ink)" }}>{t.rate}</div>
                <div className="text-xs" style={{ color: "var(--muted)" }}>per session · {t.sessions}</div>
                <div className="mt-3 pt-3 border-t" style={{ borderColor: "var(--border)" }}>
                  <div className={`${syne} text-lg font-bold`} style={{ color: "var(--success)" }}>{t.monthly}</div>
      ...
      <section className="px-4 py-16">
        <div className="max-w-4xl mx-auto">
          <div className="text-center mb-10">
            ...
            <h2 className={`${syne} font-extrabold text-2xl md:text-3xl`} style={{ color: "var(--ink)" }}>Start Mentoring in 4 Steps</h2>
          </div>
      ...
      <section className="px-4 py-16" style={{ background: "var(--primary)" }}>
        <div className="max-w-2xl mx-auto text-center">
          <h2 className={`${syne} font-extrabold text-xl md:text-2xl text-white mb-3`}>Ready to Make an Impact?</h2>
