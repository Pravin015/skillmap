    ...
    <div>
      ...
      <section className="relative px-4 flex flex-col items-center justify-center text-center" style={{ background: "#0C1A1A", minHeight: "85vh", paddingTop: "7rem", paddingBottom: "5rem" }}>
        ...
        <div className="relative max-w-[700px] mx-auto w-full">
          ...
          <p className="animate-fade-up-2 mx-auto" style={{ color: "#6B8F8F", fontSize: "clamp(1rem, 2vw, 1.15rem)", lineHeight: 1.75, maxWidth: 550, marginBottom: "2.5rem" }}>Guide India&apos;s next generation of professionals from your own experience. Set your rates, host events, build your reputation.</p>
      ...
      <section className="px-4" style={{ background: "#fff", paddingTop: "6rem", paddingBottom: "6rem" }}>
        <div className="mx-auto" style={{ maxWidth: 1200 }}>
          <div className="text-center mb-12">
            ...
            <h2 className={heading+" animate-on-scroll"} style={{color:"#0D1F1F"}}>Everything You Need to Mentor.</h2>
            <p className="animate-on-scroll mt-2 mx-auto" style={{color:"#4A6363",fontSize:"1rem",maxWidth:500}}>We handle the platform. You handle the expertise.</p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
            ...
            <div className="card-dark animate-on-scroll" style={{borderTop:"2px solid #F59E0B",padding:"2.5rem"}}>
              ...
              <p style={{color:"#6B8F8F",fontSize:"0.9rem",lineHeight:1.65,marginBottom:"1.5rem"}}>Charge Rs.300-2,000 per session for 1-on-1 or group mentoring. Switch to volunteer mode anytime. Accept bookings, share Zoom links, and get paid — all from your dashboard.</p>
              <div style={{borderRadius:"0.75rem",padding:"1rem",background:"rgba(255,255,255,0.03)"}}>
                <div style={{fontSize:"0.7rem",color:"#6B8F8F",marginBottom:"0.75rem",fontWeight:600}}>Rate Settings</div>
                {[{l:"1-on-1 Session",v:"Rs.800"},{l:"Group Session (up to 5)",v:"Rs.300/person"}].map((r)=>(<div key={r.l} className="flex justify-between items-center mb-2" style={{fontSize:"0.8rem"}}><span style={{color:"rgba(255,255,255,0.6)"}}>{r.l}</span><span style={{color:"#0ABFBC",fontWeight:600,background:"rgba(10,191,188,0.1)",padding:"0.2rem 0.5rem",borderRadius:"0.3rem",fontSize:"0.75rem"}}>{r.v}</span></div>))}
            ...
            <div className="card-dark animate-on-scroll" style={{borderTop:"2px solid #0ABFBC",padding:"2.5rem"}}>
              <div className="flex items-center gap-3 mb-4">
                <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#0ABFBC" strokeWidth="1.5"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18"/><path d="M9 21V9"/></svg>
                <span style={{background:"rgba(10,191,188,0.12)",border:"1px solid rgba(10,191,188,0.25)",borderRadius:999,padding:"0.2rem 0.7rem",fontSize:"0.7rem",fontWeight:600,color:"#0ABFBC"}}>Organised</span>
              </div>
              <h3 className={heading} style={{color:"#fff",fontSize:"1.25rem",fontWeight:700,marginBottom:"0.5rem"}}>All Your Sessions, One Place</h3>
              <p style={{color:"#6B8F8F",fontSize:"0.9rem",lineHeight:1.65,marginBottom:"1.5rem"}}>See incoming requests, accept with a Zoom link and notes, or decline with a reason. Mark sessions complete, get rated, and track your earnings.</p>
              <div style={{borderRadius:"0.75rem",padding:"1rem",background:"rgba(255,255,255,0.03)",display:"flex",flexDirection:"column",gap:"0.5rem"}}>
                <div style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:"0.5rem",padding:"0.6rem 0.75rem",fontSize:"0.75rem"}}>
                  <div className="flex justify-between"><span style={{color:"rgba(255,255,255,0.7)",fontWeight:600}}>Arjun M.</span><span style={{color:"#6B8F8F"}}>Tomorrow, 6 PM</span></div>
                  <div style={{color:"#6B8F8F",fontSize:"0.7rem",marginTop:"0.2rem"}}>Mock Interview — TCS Java</div>
                  <div className="flex gap-2 mt-2"><span style={{background:"rgba(10,191,188,0.15)",color:"#0ABFBC",padding:"0.2rem 0.5rem",borderRadius:"0.3rem",fontSize:"0.65rem",fontWeight:600}}>Accept</span><span style={{background:"rgba(255,255,255,0.05)",color:"rgba(255,255,255,0.4)",padding:"0.2rem 0.5rem",borderRadius:"0.3rem",fontSize:"0.65rem"}}>Decline</span></div>
                </div>
                <div style={{background:"rgba(10,191,188,0.08)",border:"1px solid rgba(10,191,188,0.2)",borderRadius:"0.5rem",padding:"0.6rem 0.75rem",fontSize:"0.75rem"}}>
                  <div className="flex justify-between"><span style={{color:"rgba(255,255,255,0.7)",fontWeight:600}}>Priya S.</span><span style={{color:"#0ABFBC",fontSize:"0.65rem",fontWeight:600}}>Confirmed</span></div>
                  <div style={{color:"#6B8F8F",fontSize:"0.7rem",marginTop:"0.2rem"}}>Career Roadmap — KPMG Finance</div>
                  <div style={{color:"#0ABFBC",fontSize:"0.65rem",marginTop:"0.3rem"}}>Zoom link added &#10003;</div>
                </div>
              </div>
            </div>
          </div>
      ...
      <section id="earn" style={{background:"#0C1A1A",paddingTop:"6rem",paddingBottom:"6rem"}}>
        <div className="mx-auto px-4" style={{maxWidth:1000}}>
          <div className="text-center mb-12">
            ...
            <p className="animate-on-scroll mt-1" style={{color:"#4A6363"}}>Most mentors start earning within their first week.</p>
          </div>
          <div ref={earnRef} className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {[
              ...
              <div style={{fontSize:"0.75rem",color:"#4A6363",textTransform:"uppercase",letterSpacing:"0.1em",fontWeight:600}}>{t.lbl}</div>
              <div style={{fontSize:"0.8rem",color:"#4A6363",marginTop:"0.25rem"}}>{t.exp}</div>
              ...
              <div style={{fontSize:"0.8rem",color:"#4A6363"}}>per session · {t.sess}</div>
              <div style={{borderTop:"1px solid rgba(255,255,255,0.06)",margin:"1rem 0"}}/>
              <div className={heading} style={{fontSize:t.feat?"1.25rem":"1.1rem",fontWeight:700,color:"#0ABFBC"}}>{t.monthly}+</div>
              <div style={{fontSize:"0.75rem",color:"#4A6363"}}>potential monthly</div>
            </div>))}
          </div>
          <p className="text-center mt-6" style={{color:"#4A6363",fontSize:"0.75rem"}}>*Estimates based on platform average. Actual earnings depend on sessions booked and rates set.</p>
        </div>
      </section>
      ...
      <section style={{background:"#fff",paddingTop:"6rem",paddingBottom:"6rem"}}>
        <div className="mx-auto px-4" style={{maxWidth:1000}}>
          <div className="text-center mb-14">
            ...
            <h2 className={heading+" animate-on-scroll"} style={{color:"#0D1F1F"}}>Start Mentoring in 4 Steps</h2>
            <p className="animate-on-scroll" style={{color:"#4A6363",marginTop:"0.5rem"}}>From application to first booking in days, not weeks.</p>
          </div>
          <div className="relative">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 lg:gap-6">
              {[{n:"01",t:"Apply as Mentor",d:"Fill out the mentor form with your experience, expertise, and LinkedIn profile. Takes 5 minutes."},{n:"02",t:"Get Verified",d:"Our team reviews your application. Verified mentors get a badge and appear higher in student searches."},{n:"03",t:"Set Availability & Rates",d:"Configure your session rates, availability, and whether you offer free or paid sessions."},{n:"04",t:"Start Mentoring",d:"Students book sessions. You accept, share a Zoom link, conduct the session, and get rated."}].map((s)=>(<div key={s.n} className="text-center animate-on-scroll">
                <div className="mx-auto flex items-center justify-center" style={{width:"3rem",height:"3rem",borderRadius:"50%",background:"rgba(10,191,188,0.08)",border:"1.5px solid #0ABFBC"}}><span className={heading} style={{fontSize:"0.9rem",fontWeight:700,color:"#0ABFBC"}}>{s.n}</span></div>
                <h3 className={heading} style={{color:"#0D1F1F",fontWeight:700,fontSize:"1.1rem",marginTop:"1.25rem",marginBottom:"0.5rem"}}>{s.t}</h3>
                <p className="mx-auto" style={{color:"#4A6363",fontSize:"0.9rem",lineHeight:1.6,maxWidth:220}}>{s.d}</p>
              </div>))}
            </div>
          </div>
          <div className="text-center mt-12"><p style={{color:"#6B8F8F",fontSize:"0.9rem",marginBottom:"1rem"}}>Ready to share your expertise?</p><Link href="/forms/mentor-onboarding" className="btn-primary no-underline" style={{padding:"0.9rem 2rem",fontSize:"1rem"}}>Apply as Mentor — Free &#8594;</Link></div>
        </div>
      </section>
      ...
      <section className="relative px-4 text-center" style={{background:"#0C1A1A",paddingTop:"7rem",paddingBottom:"7rem"}}>
        ...
        <div className="relative mx-auto" style={{maxWidth:650}}>
          ...
          <p className="animate-on-scroll" style={{color:"#4A6363",marginBottom:"2rem"}}>Join verified mentors helping students land their dream jobs.</p>
          <Link href="/forms/mentor-onboarding" className="btn-primary animate-glow no-underline" style={{padding:"0.9rem 2.2rem",fontSize:"1rem"}}>Apply as Mentor — Free &#8594;</Link>
          <div className="flex justify-center gap-2 mt-4 flex-wrap">
            {["&#128274; Verified Only","&#128176; You Set Rates","&#127470;&#127475; Built for India"].map((b,i)=>(<span key={i} style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)",borderRadius:999,padding:"0.3rem 0.8rem",color:"#6B8F8F",fontSize:"0.75rem"}} dangerouslySetInnerHTML={{__html:b}}/>))}
          </div>
        </div>
      </section>
    </div>
