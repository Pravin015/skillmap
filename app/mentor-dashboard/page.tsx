  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)]">
      ...
      <div className="flex-1 px-4 md:px-8 py-8 pb-24 lg:pb-8 max-w-5xl">
          ...
          <div className="space-y-6">
            ...
            {pending.length > 0 && (
              <div className="rounded-2xl border bg-white p-6" style={{ borderColor: "var(--border)" }}>
                <h3 className={`${heading} font-bold text-base mb-4`}>Pending Requests ({pending.length})</h3>
                {pending.map((s) => (
                  <div key={s.id} className="flex items-center gap-4 p-3 rounded-xl border mb-2" style={{ borderColor: "var(--border)" }}>
                    <div className="flex-1"><div className={`${heading} font-bold text-sm`}>{s.student?.name}</div><div className="text-xs" style={{ color: "var(--muted)" }}>{s.sessionType} · {new Date(s.preferredDate).toLocaleDateString()} · {s.duration}{s.message ? ` · "${s.message}"` : ""}</div></div>
                    <button onClick={() => setActiveTab("sessions")} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem]`} style={{ background: "var(--primary)", color: "white" }}>Respond</button>
                  </div>
                ))}
              </div>
            )}
            <div className="flex gap-3"><Link href="/events/create" className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm no-underline`} style={{ background: "var(--primary)", color: "white" }}>+ Create Event</Link></div>
          </div>
        )}
        ...
        {activeTab === "sessions" && (
          <div className="space-y-6">
            <div><h2 className={`${heading} font-bold text-xl`}>Session Requests</h2><p className="text-sm mt-1" style={{ color: "var(--muted)" }}>{sessions.length} total sessions</p></div>
            {sessions.length === 0 ? (
              <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}><div className="text-4xl mb-3">📅</div><p className={`${heading} font-bold text-base`}>No sessions yet</p><p className="text-sm" style={{ color: "var(--muted)" }}>Students will appear here when they request sessions</p></div>
            ) : (
                  ...
                  <div key={s.id} className="rounded-2xl border bg-white p-5" style={{ borderColor: "var(--border)" }}>
                    <div className="flex items-start gap-4 flex-wrap">
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2"><span className={`${heading} font-bold`}>{s.student?.name || "Student"}</span><span className={`text-[0.6rem] font-bold px-2 py-0.5 rounded-full ${statusBadge[s.status]}`}>{s.status}</span><span className={`text-[0.6rem] font-bold px-2 py-0.5 rounded-full ${s.sessionType === "GROUP" ? "bg-[#EDE9FE] text-[#7C3AED]" : "bg-blue-100 text-blue-700"}`}>{s.sessionType === "GROUP" ? "Group" : "1-on-1"}</span></div>
                      ...
                      <div className="flex gap-2 shrink-0">
                          ...
                          <>
                            <div className="flex flex-col gap-2">
                              ...
                              <div className="flex gap-2">
                                <button onClick={() => handleAction(s.id, "accept", { meetingLink: acceptLink.link, mentorNotes: acceptLink.notes })} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem]`} style={{ background: "var(--primary)", color: "white" }}>Accept</button>
                        ...
                        {s.status === "ACCEPTED" && <button onClick={() => handleAction(s.id, "complete")} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem]`} style={{ background: "var(--primary)", color: "white" }}>Mark Complete</button>}
        ...
        {activeTab === "events" && (
          <div className="space-y-6">
            <div className="flex items-center justify-between"><div><h2 className={`${heading} font-bold text-xl`}>My Events</h2><p className="text-sm mt-1" style={{ color: "var(--muted)" }}>{events.length} events</p></div>
              <Link href="/events/create" className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm no-underline`} style={{ background: "var(--primary)", color: "white" }}>+ Create</Link></div>
            {events.length === 0 ? (
              <div className="rounded-2xl border bg-white p-12 text-center" style={{ borderColor: "var(--border)" }}><div className="text-4xl mb-3">🎤</div><p className={`${heading} font-bold text-base`}>No events</p><Link href="/events/create" className={`inline-block mt-3 px-4 py-2 rounded-lg ${heading} font-bold text-xs no-underline`} style={{ background: "var(--primary)", color: "white" }}>Create your first event</Link></div>
            ) : (
              <div className="space-y-3">{events.map((e) => (
                <div key={e.id} className="rounded-2xl border bg-white p-5 flex items-center gap-4 flex-wrap" style={{ borderColor: "var(--border)" }}>
                  <div className="flex-1 min-w-0"><div className={`${heading} font-bold`}>{e.title}</div><div className="text-xs" style={{ color: "var(--muted)" }}>{new Date(e.date).toLocaleDateString()} · {e.pricing === "FREE" ? "Free" : `₹${(e.price || 0) / 100}`} · {e._count.registrations} registered</div></div>
                  ...
                  <div className="flex gap-2 shrink-0">
                    <button onClick={() => showAttendees(e.id)} className={`px-3 py-1.5 rounded-lg ${heading} font-bold text-[0.7rem]`} style={{ background: "var(--primary)", color: "white" }}>Attendees ({e._count.registrations})</button>
        ...
        {activeTab === "profile" && (
          <div className="space-y-6">
            <div><h2 className={`${heading} font-bold text-xl`}>My Mentor Profile</h2></div>
            <div className="flex gap-3 flex-wrap">
              <Link href="/profile/edit" className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm no-underline`} style={{ background: "var(--primary)", color: "white" }}>Edit Profile</Link>
              <Link href="/settings" className={`px-4 py-2.5 rounded-xl ${heading} font-bold text-sm no-underline border`} style={{ borderColor: "var(--border)", color: "var(--ink)" }}>Account Settings</Link>
            </div>
          </div>
        )}
      </div>
