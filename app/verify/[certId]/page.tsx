...
export default function VerifyCertPage({ params }: { params: Promise<{ certId: string }> }) {
  ...
  if (loading) return <div className="min-h-screen flex items-center justify-center" style={{ background: "#0C1A1A" }}><div className="h-8 w-8 animate-spin rounded-full border-4 border-t-transparent" style={{ borderColor: "var(--primary)", borderTopColor: "transparent" }} /></div>;

  if (notFound) return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: "#0C1A1A" }}>
      <div className="text-center">
        <div className="text-5xl mb-4">❌</div>
        <h1 className={`${heading} text-xl font-bold text-white mb-2`}>Certificate Not Found</h1>
        <p style={{ color: "#6B8F8F" }}>The certificate ID <strong className="text-white">{certId}</strong> does not exist or is invalid.</p>
      </div>
    </div>
  );

  return (
    <div className="min-h-screen flex items-center justify-center px-4" style={{ background: "#0C1A1A" }}>
      <div className="max-w-lg w-full">
        ...
        <div className="card-dark text-center" style={{ padding: "3rem 2rem", borderTop: "3px solid var(--primary)" }}>
          ...
          <p className="text-sm mb-6" style={{ color: "#6B8F8F" }}>Completed by</p>

          <div className={`${heading} text-2xl font-bold mb-1`} style={{ color: "var(--primary)" }}>{cert!.userName}</div>

          <div className="mt-6 pt-4" style={{ borderTop: "1px solid rgba(255,255,255,0.06)" }}>
            <div className="flex justify-between text-xs" style={{ color: "#4A6363" }}>
              <span>Certificate ID</span>
              <span className="font-mono text-white">{cert!.certificateId}</span>
            </div>
            <div className="flex justify-between text-xs mt-2" style={{ color: "#4A6363" }}>
              <span>Issued On</span>
          ...
          <div className="mt-6 rounded-lg p-3" style={{ background: "rgba(10,191,188,0.08)", border: "1px solid rgba(10,191,188,0.2)" }}>
            <p className="text-xs font-medium" style={{ color: "#0ABFBC" }}>✓ This certificate is authentic and verified by AstraaHire.</p>
          </div>
        </div>
      </div>
    </div>
