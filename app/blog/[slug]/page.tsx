  ...
  return (
    <div className="min-h-[calc(100vh-4rem)]" style={{ background: "var(--surface)" }}>
      <article className="max-w-3xl mx-auto px-4 py-8">
        ...
        {/* Title */}
        <h1 className={`${syne} font-extrabold text-2xl md:text-4xl mb-4`} style={{ color: "var(--ink)" }}>{post.title}</h1>

        {/* Meta */}
        <div className="flex items-center gap-4 mb-8 text-sm" style={{ color: "var(--muted)" }}>
          <span className={`${syne} font-bold`}>{post.authorName}</span>
        ...
        <div className="mt-8 flex items-center gap-3">
          ...
          <button onClick={() => { navigator.clipboard.writeText(window.location.href); alert("Link copied!"); }} className={`px-4 py-2 rounded-xl ${syne} font-bold text-xs border`} style={{ borderColor: "var(--border)", color: "var(--ink)" }}>📋 Copy link</button>
        </div>
      </article>
    </div>
  );
