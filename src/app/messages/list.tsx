  ...
  return (
    <div className="divide-y divide-line overflow-hidden rounded-2xl border border-line bg-surface/60">
      {convos.map((c) => {
        ...
        return (
          <Link key={c.id} href={`/messages/${c.id}`} className={`flex items-center gap-3 px-4 py-3 hover:bg-white/[0.03] ${activeId === c.id ? "bg-cyan/[0.05]" : ""}`}>
            ...
            <div className="min-w-0 flex-1">
              ...
              {c.requirement ? <p className="truncate text-[11px] text-[#b79cff]">{c.requirement.title}</p> : null}
