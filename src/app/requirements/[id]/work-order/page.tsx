        ...
        <>
          <Card className="p-8 print:border-0 print:shadow-none">
            ...
            <div className="mt-8 grid gap-6 border-t border-line pt-5 text-sm md:grid-cols-2">
              ...
              <div><p className="mono text-[11px] uppercase tracking-wider text-muted">Signed by the trainer</p>{wo.trainerSignedName ? <><p className="mt-1 font-display text-xl italic">{wo.trainerSignedName}</p><p className="text-xs text-muted">{awarded.trainer.user.name} · {fmtDate(wo.trainerSignedAt!)}{wo.signatureHash ? <span className="mono"> · ref {wo.signatureHash.slice(0, 12)}</span> : null}{wo.ndaAcceptedAt ? " · NDA accepted" : ""}</p></> : <p className="mt-1 text-muted">Not yet accepted</p>}</div>
            </div>
          </Card>
          ...
          <div className="flex flex-wrap gap-2 print:hidden">
            ...
            {wo.status === "ACCEPTED" ? <Link href={`/requirements/${id}/work-order/attendance`} className="inline-flex h-9 items-center rounded-lg border border-line-2 bg-white px-3 font-display text-sm font-semibold hover:bg-surface-2">Attendance & records</Link> : null}
          ...
          {isTrainer && wo.status === "SENT" ? (
            <Card className="p-6 print:hidden" glow="cyan">
              ...
              <ActionForm action={respondWorkOrder} className="mt-4 space-y-3">
                ...
                {req.company.ndaText ? <details className="rounded-lg border border-line bg-surface-2/60 p-3 text-sm"><summary className="cursor-pointer font-display font-semibold">Non-disclosure agreement from {req.company.name}</summary><p className="mt-2 whitespace-pre-line text-ink/90">{req.company.ndaText}</p></details> : null}
                {req.company.ndaText ? <label className="flex items-start gap-2 text-sm"><input type="checkbox" name="nda" value="1" className="mt-0.5 accent-cyan" /> I accept the NDA above; it is recorded with my signature.</label> : null}
