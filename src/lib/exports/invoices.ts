...
export type InvoiceRow = {
  ...
  customerName: string; customerGstin: string | null; customerAddress: string | null; supplierName: string; supplierGstin: string | null;
};
...
export function zohoBooksCsv(rows: InvoiceRow[]) {
  const cols = ["Invoice Number", "Invoice Date", "Due Date", "Customer Name", "GST Treatment", "GST Identification Number (GSTIN)", "Place of Supply", "Invoice Status", "Currency Code", "Item Name", "Item Desc", "Quantity", "Item Price", "Item Tax %", "Item Tax Amount", "Item Total", "SubTotal", "Total", "Notes"];
  const lines = rows.map((r) => [
    ...
    "Corporate training", r.description, 1, r.amount, r.gstRate, r.gstAmount, r.total, r.amount, r.total, `Imported from CorpGurus · supplier ${r.supplierName}${r.supplierGstin ? ` (${r.supplierGstin})` : ""}`,
  ].map(esc).join(","));
...
export function tallyXml(rows: InvoiceRow[], companyName: string) {
  const vouchers = rows.map((r) => {
    ...
    const interstate = r.customerGstin && r.supplierGstin && r.customerGstin.slice(0, 2) !== r.supplierGstin.slice(0, 2);
...
<PARTYLEDGERNAME>${xml(r.customerName)}</PARTYLEDGERNAME><PARTYGSTIN>${xml(r.customerGstin ?? "")}</PARTYGSTIN><NARRATION>${xml(r.description)} (CorpGurus)</NARRATION><ISINVOICE>Yes</ISINVOICE>
